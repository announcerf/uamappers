...
impl MapperStatsCurrentRepo {
            ...
            .on_conflict(
                OnConflict::column(mapper_stats_current::Column::OsuUserId)
                    .update_columns([
                        ...
                        mapper_stats_current::Column::AvgRating,
                        mapper_stats_current::Column::WeightedRating,
                        mapper_stats_current::Column::AvgStars,
                        mapper_stats_current::Column::MinStars,
                        mapper_stats_current::Column::MaxStars,
                        mapper_stats_current::Column::AvgBpm,
                        mapper_stats_current::Column::AvgLengthSeconds,
                        mapper_stats_current::Column::AvgAr,
                        mapper_stats_current::Column::AvgCs,
                        mapper_stats_current::Column::AvgOd,
                        mapper_stats_current::Column::AvgHp,
                        ...
                        mapper_stats_current::Column::HasRanked,
                        mapper_stats_current::Column::HasLoved,
                        mapper_stats_current::Column::HasGuest,
                        mapper_stats_current::Column::HasNominated,
    ...
    fn to_active(&self, row: NewMapperStatsCurrentRow) -> mapper_stats_current::ActiveModel {
        ...
        mapper_stats_current::ActiveModel {
            ...
            weighted_rating: Set(row.weighted_rating),
            avg_stars: Set(row.avg_stars),
            min_stars: Set(row.min_stars),
            max_stars: Set(row.max_stars),
            avg_bpm: Set(row.avg_bpm),
            avg_length_seconds: Set(row.avg_length_seconds),
            avg_ar: Set(row.avg_ar),
            avg_cs: Set(row.avg_cs),
            avg_od: Set(row.avg_od),
            avg_hp: Set(row.avg_hp),
            ...
            has_ranked: Set(row.has_ranked),
            has_loved: Set(row.has_loved),
            has_guest: Set(row.has_guest),
            has_nominated: Set(row.has_nominated),
            updated_at: Set(now),
        }
    }
}
