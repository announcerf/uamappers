...
use super::super::projection::user_to_mapper_fingerprint;
...
impl MapperEnrich {
    pub(in super::super) async fn persist_mapper_user(
        ...
        let fingerprint = serde_json::to_value(user_to_mapper_fingerprint(&extended, fetched_at))
            .unwrap_or(serde_json::Value::Null);

        persist_user_profile(self, mapper.osu_user_id, fingerprint, fetched_at).await?;
